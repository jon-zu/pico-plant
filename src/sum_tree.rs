    ...
    Sample: Copy + Add<Output = Sample>,
{
    ...
    pub fn init_leafs(&mut self, v: Sample) {
        for i in 0..Self::leaf_count() {
            self.update_leaf_node_sample(i, v);
        }
    }
}
    ...
    Sample: Copy,
{
    pub fn new(zero: Sample) -> Self {
        ...
        //let num_leaf_nodes = 2 * N.checked_next_power_of_two().unwrap();
        Self { nodes: [zero; N] }
    }
}
...
mod tests {
    ...
    #[test]
    fn basics() {
        let mut sum_tree = SumTree::<i32, 6>::new(0);
